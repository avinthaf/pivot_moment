import { Heading, Text, TextField, Button } from '@radix-ui/themes';
import { Link } from 'react-router';

const Signup = () => {
  return (
    <main className="flex items-center justify-center w-full px-4 min-h-screen bg-gradient-to-b from-[#F7FAF7] via-[#F0FDF4] to-[#DCFCE7]">
      <form className="flex w-full flex-col max-w-96 bg-white rounded-4xl p-6 shadow-md">
        <a href="/" className="mb-8" title="Go to Home">
          <svg className="size-10" width="30" height="33" viewBox="0 0 30 33" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="m8 4.55 6.75 3.884 6.75-3.885M8 27.83v-7.755L1.25 16.19m27 0-6.75 3.885v7.754M1.655 8.658l13.095 7.546 13.095-7.546M14.75 31.25V16.189m13.5 5.976V10.212a2.98 2.98 0 0 0-1.5-2.585L16.25 1.65a3.01 3.01 0 0 0-3 0L2.75 7.627a3 3 0 0 0-1.5 2.585v11.953a2.98 2.98 0 0 0 1.5 2.585l10.5 5.977a3.01 3.01 0 0 0 3 0l10.5-5.977a3 3 0 0 0 1.5-2.585"
              stroke="#1d293d" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </a>

        <Heading size="8" className="text-gray-900">Sign up</Heading>

        <Text className="mt-4 text-gray-500">
          Create your account to get started.
        </Text>

        <div className="mt-10">
          <Text as="label" weight="medium">Full Name</Text>
          <TextField.Root
            size="3"
            placeholder="Please enter your full name"
            className="mt-2 w-full"
            required
            type="text"
            name="name"
          />
        </div>

        <div className="mt-6">
          <Text as="label" weight="medium">Email</Text>
          <TextField.Root
            size="3"
            placeholder="Please enter your email"
            className="mt-2 w-full"
            required
            type="email"
            name="email"
          />
        </div>

        <div className="mt-6">
          <Text as="label" weight="medium">Password</Text>
          <TextField.Root
            size="3"
            placeholder="Please enter your password"
            className="mt-2 w-full"
            required
            type="password"
            name="password"
          />
        </div>

        <div className="mt-6 mb-8">
          <Text as="label" weight="medium">Confirm Password</Text>
          <TextField.Root
            size="3"
            placeholder="Please confirm your password"
            className="mt-2 w-full"
            required
            type="password"
            name="confirmPassword"
          />
        </div>

        <Button
          type="submit"
          className="w-full cursor-pointer"
          size="3"
        >
          Sign Up
        </Button>

        <Text className="text-center py-8">
          Already have an account?{' '}
          <Link to="/auth/login" className="text-indigo-600 hover:underline">
            Sign in
          </Link>
        </Text>
      </form>
    </main>
  );
};

export default Signup;
